...
- Docker images tagged `latest` + version (e.g., `a2go:v0.2.1`) are only built on releases (when a `v*` tag is created). PRs build with branch-based tags (branch name sanitized: slashes/underscores → hyphens, lowercased). E.g. `feat/update-template` → `a2go:feat-update-template`. Use branch tags to test on RunPod before merging.
